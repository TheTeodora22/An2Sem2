...
class GeneticAlgorithm:
    def __init__(self, n, a, b, coef, precision, pc, pm, steps):
        self.n = n  
        self.a = a  
        self.b = b  
        self.coef = coef  
    ...
    def run(self, output_file):
        with open(output_file, "w") as fout:
            ...
            for i, ind in enumerate(self.population):
                fout.write(f"{i+1}: {ind['bits']} x= {ind['x']:.6f} f= {ind['f']:.6f}\n")
            ...
            fout.write("\nProbabilitati selectie:\n")
            for i, p in enumerate(p_sel):
                fout.write(f"cromozom {i+1} probabilitate {p}\n")

            q = np.cumsum([0] + p_sel)
            fout.write("\nIntervale probabilitati selectie:\n")
            fout.write(" ".join([f"{val:.6f}" for val in q]) + "\n\n")
            ...
            for _ in range(self.n):
                ...
                idx = self.binary_search_selection(q[1:], u)
                selected_pop.append(self.population[idx].copy())
                fout.write(f"u={u:.6f} selectam cromozomul {idx+1}\n")

            fout.write("\nDupa selectie:\n")
            for i, ind in enumerate(selected_pop):
                fout.write(f"{i+1}: {ind['bits']} x= {ind['x']:.6f} f= {ind['f']:.6f}\n")

            fout.write(f"\nProbabilitatea de incrucisare {self.pc}\n")
            to_cross = []
            for i in range(self.n):
                u = np.random.random()
                if u < self.pc:
                    fout.write(f"{i+1}: {selected_pop[i]['bits']} u={u:.6f} < {self.pc} participa\n")
                    to_cross.append(i)
                else:
                    fout.write(f"{i+1}: {selected_pop[i]['bits']} u={u:.6f}\n")

            np.random.shuffle(to_cross)
            for i in range(0, len(to_cross) - 1, 2):
                idx1, idx2 = to_cross[i], to_cross[i+1]
                p_cut = np.random.randint(1, self.m)
                ...
                fout.write(f"Recombinare dintre cromozomul {idx1+1} cu {idx2+1}:\n")
                fout.write(f"{b1} {b2} taiat in {p_cut}\nRezultat: {new_b1} {new_b2}\n")
            ...
            fout.write(f"\nProbabilitate de mutatie pentru fiecare gena {self.pm}\nAu fost modificati cromozomii:\n")
            for i in range(self.n):
                mutated = False
                bits_list = list(selected_pop[i]['bits'])
                for j in range(self.m):
            ...
            for step in range(2, self.steps + 1):
                current_max = max(ind['f'] for ind in self.population)
                current_avg = sum(ind['f'] for ind in self.population) / self.n
                fout.write(f"Generatia {step}: Max={current_max:.10f}, Media={current_avg:.10f}\n")
                ...
                to_cross = [i for i in range(self.n) if np.random.random() < self.pc]
                np.random.shuffle(to_cross)
                for i in range(0, len(to_cross)-1, 2):
                    p_cut = np.random.randint(1, self.m)
                    i1, i2 = to_cross[i], to_cross[i+1]
...
if __name__ == "__main__":
    with open("input.txt", "r") as fin:
        ...
        a, b = float(line2[0]), float(line2[1])
        coef = list(map(float, fin.readline().split()))
        precision = int(fin.readline().strip())
        pc = float(fin.readline().strip())
        pm = float(fin.readline().strip())
        steps = int(fin.readline().strip())

    ga = GeneticAlgorithm(n, a, b, coef, precision, pc, pm, steps)
>>>>>>> Stashed changes
    ga.run("Evolutie.txt")